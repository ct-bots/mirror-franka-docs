...
# Custom html target that first imports repos, generates JS, clones franka_ros2 and franka_toolbox_for_matlab, then builds
# Custom html target that first imports repos, generates JS, clones franka_ros2 and franka_toolbox_for_matlab, then builds
html: generate-js
	@echo "Importing documentation repositories..."
	@if [ ! -d "$(SOURCEDIR)/franka_ros2" ] || [ ! -d "$(SOURCEDIR)/franka_toolbox_for_matlab" ]; then \
